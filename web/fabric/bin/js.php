	...
	public function web_common($return_real_path = false, $return_ash = false){
		...
		if(ll('files')->file_exists($file_cache)&&is_file($file_cache)){
			if($return_real_path){
				...
				//$file_cache = $ll_store->get_subsite_id().md5(serialize($file).$stats['ctime']).'.js';
...
//					lc('cache')->set($file_cache, $content, true, 7200);
	...
	public function web_load($return_ash = false){
		//this load all the JS necessary for this page in one time
		$config	 = lc('config')->get_and_unload_config('display');
		$scripts = isset($config['script'])?$config['script']:array();
		$uri	 = $this->config['uri'];
		$return	 = '';
		if(is_array($scripts)){
			$files	 = array();
			...
			foreach($scripts as $script){
				if(
					...
					$files['time_'.$script['src']]	 = filemtime($this->web_common(true));
				}
			}
			if(is_null($old_f)){
				$uri->delete('f');
