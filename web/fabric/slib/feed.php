...
class feed {
	...
	private function getCacheFilename() {
		return TMPPATH . '/' . md5($this->url) . '.feed.cache';
//		return sys_get_temp_dir() . '/' . md5($this->url) . '.feed.cache';
	 ...
	 */
	public function feedcreator(){
		return ll('feedcreator'.DIRECTORY_SEPARATOR.'feedcreator');
	}
...
class RSSReader implements FeedReader {
	...
	public function item($index) {
		...
		return (object)array(
			'title'			=> (string)$node->title,
			'description'	=> (string)$node->description,
			'content'		=> (string)$node->children('content',true),
